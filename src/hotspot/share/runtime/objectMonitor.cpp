...
bool ObjectMonitor::enter(JavaThread* current) {
  ...
  enter_with_contention_mark(current);
...

  {
    ...
    for (;;) {
      ExitOnSuspend eos(this);
      {
        ...
        enter_or_reenter_internal(current);
        current->set_current_pending_monitor(nullptr);
...
void ObjectMonitor::enter_internal_self_park(JavaThread* current, bool do_timed_parked) {
...
void ObjectMonitor::reenter_internal_self_park(JavaThread* current) {
  OSThreadContendState osts(current->osthread());
  assert(current->thread_state() == _thread_in_vm, "invariant");
  {
...
// This method is a result of reconciliation of enter_internal() and reenter_internal()
void ObjectMonitor::enter_or_reenter_internal(JavaThread* current, ObjectWaiter* currentNode) {
  // Reenter path requires currentNode != nullptr, whereas enter path does not need it at all
  const bool reenterPath = currentNode != nullptr;
  ...
  for (;;) {
    ...
    // is necessary.
    if (reenterPath && try_spin(current)) {
      break;
    }

    if (!reenterPath) {
      enter_internal_self_park(current, do_timed_parked);
    } else {
      reenter_internal_self_park(current);
    }
  ...
  if (!reenterPath) {
    ...
    OrderAccess::fence();      // see comments at the end of previous case
  }
}
...
void ObjectMonitor::wait(jlong millis, bool interruptible, TRAPS) {
  ...
  { // State transition wrappers
    ...
    if (v == ObjectWaiter::TS_RUN) {
      ...
      guarantee(v == ObjectWaiter::TS_ENTER, "invariant");
      enter_or_reenter_internal(current, &node);
